...
public class DynamicReloader implements Runnable {
    ...
    private void reloadApp(AppReloadInfo appInfo) throws IOException {
        ...
        deployParam.set(DeploymentProperties.KEEP_REPOSITORY_DIRECTORY, Boolean.TRUE.toString());
        ...
        if (appInfo.getContextRoot() != null && !appInfo.getContextRoot().isEmpty()) {
            deployParam.set(DeployCommandParameters.ParameterNames.CONTEXT_ROOT, appInfo.getContextRoot());
        }
        if (keepState) {
            deployParam.set(DeployCommandParameters.ParameterNames.KEEP_STATE, Boolean.TRUE.toString());
        }
    ...
    private final static class AppReloadInfo {
        ...
        /** application info */
        private final String name, contextRoot;
