 ...
 * holder.
 */
package fish.payara.extensions.notifiers.compat.newrelic;
 ...
 *
 * @deprecated folded into {@link fish.payara.extensions.notifiers.newrelic.SetNewRelicNotifierConfigurationCommand}
 * @author mertcaliskan
 */
@Deprecated
...
public class NewRelicNotificationConfigurer extends BaseSetNotifierConfigurationCommand {
    ...
    protected void configureNotifier(AdminCommandContext context, String commandName) {
        ...
        if (StringUtils.ok(key)) {
            parameterMap.insert("key", key);
        }

        if (StringUtils.ok(accountId)) {
            parameterMap.insert("accountId", accountId);
        }
